...
import { BookOpenCheck, Home, LayoutDashboard, PanelLeft, Settings, ShoppingBasket, User, Wallet } from "lucide-react";
...
export default function UserDashboard({ children }: Readonly<{ children: React.ReactNode }>) {
      ...
      <div className="flex flex-col sm:gap-4 sm:py-4 sm:pl-14">
        <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b bg-background px-4 sm:static sm:h-auto sm:border-0 sm:bg-transparent sm:px-6">
          ...
          {navigation.map(
            (item) =>
              item.href === pathName && (
                <h1 key={item.title} className="hidden md:block md:font-bold md:text-xl">
                  {capitalizedTitle}
                </h1>
              )
          )}
